  ...
  return (
    <Card className="bg-white/90 backdrop-blur-sm border border-gray-100 rounded-2xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-sm sm:text-base font-medium text-gray-900 truncate pr-2">
          {title}
        </CardTitle>
        <div className="h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-gray-50 flex items-center justify-center flex-shrink-0">
          <BarChart2 className="h-3 w-3 sm:h-4 sm:w-4 text-gray-500" />
        </div>
      </CardHeader>
      <CardContent>
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="flex flex-col xs:flex-row xs:justify-between gap-2 xs:gap-0">
            <div className="flex items-center gap-2 text-gray-500">
              ...
              <span className="text-xs sm:text-sm">Due {dueDate}</span>
            </div>
            <div className="flex -space-x-2">
              {team.slice(0, 3).map((member, i) => (
                <Avatar
                  key={i}
                  className="h-6 w-6 sm:h-7 sm:w-7 border-2 border-white shadow-sm"
                >
              ...
              {team.length > 3 && (
                <div className="h-6 w-6 sm:h-7 sm:w-7 rounded-full bg-gray-100 border-2 border-white shadow-sm flex items-center justify-center">
      ...
      <div className="p-4 sm:p-6 h-full">
        <div className="responsive-grid">
          {[...Array(6)].map((_, index) => (
            <Card
              key={index}
              className="bg-white/90 backdrop-blur-sm border border-gray-100 rounded-2xl shadow-sm h-[180px] sm:h-[220px] flex items-center justify-center"
            >
        ...
        <Card className="bg-white/90 backdrop-blur-sm border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <div className="h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-blue-50 flex items-center justify-center">
              <BarChart2 className="h-3 w-3 sm:h-4 sm:w-4 text-blue-500" />
            </div>
          </CardHeader>
          <CardContent>
