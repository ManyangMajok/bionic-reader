  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Welcome to Bionic Reader! You now have full access.",
      });

      // Always redirect to bionic reader after signup
    ...
    <AuthLayout>
      <div className="bg-white rounded-2xl shadow-sm p-6 sm:p-8 w-full">
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
          ...
          <div className="text-xs sm:text-sm text-center text-gray-500 mt-4 sm:mt-6 leading-relaxed">
            By creating an account, you agree to our{" "}
            <Link
              to="/"
              className="text-blue-600 hover:underline mobile-tap-target transition-colors"
            >
              Terms of Service
            </Link>{" "}
            and{" "}
